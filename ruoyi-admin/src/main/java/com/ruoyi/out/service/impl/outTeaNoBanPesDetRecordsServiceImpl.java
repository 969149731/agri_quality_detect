...
public class outTeaNoBanPesDetRecordsServiceImpl implements IoutTeaNoBanPesDetRecordsService 
{
    ...
    public List<outReturnType> selectoutTeaNoBanPesDetRecordsList(outTeaNoBanPesDetRecords outTeaNoBanPesDetRecords)
    {
        ...
        List<String> pesticideList = outTeaNoBanPesDetRecordsMapper.getTeaNoBanPesticideList();//可以在此处设置农药列表//也可查询获取列表
        ...
        for (outFruVegSelectType item : SelectList) {
            ...
            */
            ...
            //获取对应标准//在这里可以获取多种标准
            PageHelper.startPage(0,0,false,false,true);//分页方法，仅对之后第一个查询生效
            List<agriPesticideResidueStandard> standardslist = outTeaNoBanPesDetRecordsMapper.getagriPesticideResidueStandard(pesticideName, vegFruName);
