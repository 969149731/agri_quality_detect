...
public class outFruVegSelectType2 {//比原始的多一个国家标准的属性
    ...
    public void fixData(){//数据预处理，目前主要是对生产环节进行纠正
        ...
        for (String type : StageType){
            if(this.samplingStageType!=null && this.samplingStageType.contains(type)){
                this.samplingStageType="生产基地";//上述均为生产基地
                return;//找到一个即可返回
            }
        }
        if(this.samplingStageType!=null && this.samplingStageType.equals("基地")){//不是上述类型，但是包含生产基地，应为其他基地类型
            this.samplingStageType="生产基地";//将所有数据清洗为规范格式
            return;//找到一个即可返回
        }
        ...
        if(this.samplingStageType!=null && this.samplingStageType.contains("市场")){//不是上述类型，但是包含生产基地，应为其他基地类型
            this.samplingStageType="批发市场";//将所有数据清洗为规范格式
            return;//找到一个即可返回
        }
