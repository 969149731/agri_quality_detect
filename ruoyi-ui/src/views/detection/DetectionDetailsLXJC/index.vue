...
<!--      </el-form-item>-->
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="检测类型" prop="samplingType">
          <!-- 将输入框禁用，使用户无法修改 -->
          <el-input v-model="form.samplingType" placeholder="例行检测" disabled />
        </el-form-item>
...
} from "@/api/detection/DetectionDetailsLXJC";
...
export default {
  ...
  dicts: ['pass_or_not','veg_fru_type','sampling_type'],
  data() {
    return {
      ...
      upload: {
        ...
        // 上传的地址
        url: process.env.VUE_APP_BASE_API + "/detection/DetectionDetailsLXJC/importData"
      },
      // 表单参数
      form: {
        samplingType: '例行检测'  // 将值初始化为“例行检测”
      },
      // 表单校验
      rules: {
      }
  ...
  methods: {
    ...
    getList() {
      ...
      listDetectionDetailsLXJC(this.addDateRange(this.queryParams, this.dateRange))
        .then(response => {
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('detection/DetectionDetailsLXJC/export', {
        ...this.queryParams
