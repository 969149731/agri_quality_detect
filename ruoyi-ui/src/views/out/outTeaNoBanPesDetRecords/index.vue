  ...
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="氟氯氰菊酯" prop="flucycloxuron">
        <el-input
          v-model="queryParams.flucycloxuron"
          placeholder="请输入氟氯氰菊酯"
          clearable
          @keyup.enter.native="handleQuery"
        />
      </el-form-item>
      ...
      <el-form-item label="灭幼脲" prop="diflubenzuron">
        <el-input
          v-model="queryParams.diflubenzuron"
          placeholder="请输入灭幼脲"
          clearable
          @keyup.enter.native="handleQuery"
        />
      ...
      <el-form-item label="灭蝇胺" prop="cyromazine">
        <el-input
          v-model="queryParams.cyromazine"
          placeholder="请输入灭蝇胺"
          clearable
          @keyup.enter.native="handleQuery"
        />
      ...
      <el-form-item label="霜霉威" prop="metalaxylM">
        <el-input
          v-model="queryParams.metalaxylM"
          placeholder="请输入霜霉威"
          clearable
          @keyup.enter.native="handleQuery"
        />
      ...
      <el-form-item label="氯吡脲" prop="chlorantraniliprole">
        <el-input
          v-model="queryParams.chlorantraniliprole"
          placeholder="请输入氯吡脲"
          clearable
          @keyup.enter.native="handleQuery"
        />
      ...
      <el-form-item label="呋虫胺" prop="flonicamid">
        <el-input
          v-model="queryParams.flonicamid"
          placeholder="请输入呋虫胺"
          clearable
          @keyup.enter.native="handleQuery"
        />
      ...
      <el-form-item label="抑霉唑" prop="difenoconazole">
        <el-input
          v-model="queryParams.difenoconazole"
          placeholder="请输入抑霉唑"
          clearable
          @keyup.enter.native="handleQuery"
        />
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['out:outTeaNoBanPesDetRecords:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['out:outTeaNoBanPesDetRecords:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['out:outTeaNoBanPesDetRecords:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['out:outTeaNoBanPesDetRecords:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="outTeaNoBanPesDetRecordsList" @selection-change="handleSelectionChange">
      <el-table-column type="selection" width="55" align="center" />
      <el-table-column label="唯一标识每条检测记录" align="center" prop="teaNoBanPesDetRecordsId" />
      ...
      <el-table-column label="氟氯氰菊酯" align="center" prop="flucycloxuron" />
      ...
      <el-table-column label="三唑酮" align="center" prop="triazolone" />
      <el-table-column label="百菌清" align="center" prop="carbendazim" />
      <el-table-column label="腐霉利" align="center" prop="metalaxyl" />
      <el-table-column label="丙溴磷" align="center" prop="profenofos" />
      <el-table-column label="多菌灵" align="center" prop="carbaryl" />
      ...
      <el-table-column label="氟啶脲" align="center" prop="fludioxonil" />
      <el-table-column label="灭幼脲" align="center" prop="diflubenzuron" />
      <el-table-column label="灭蝇胺" align="center" prop="cyromazine" />
      <el-table-column label="噻虫嗪" align="center" prop="thiamethoxam" />
      <el-table-column label="甲霜灵" align="center" prop="mancozeb" />
      <el-table-column label="霜霉威" align="center" prop="metalaxylM" />
      <el-table-column label="吡唑醚菌酯" align="center" prop="pyraclostrobin" />
      <el-table-column label="氯吡脲" align="center" prop="chlorantraniliprole" />
      <el-table-column label="嘧霉胺" align="center" prop="azoxystrobin" />
      <el-table-column label="吡虫啉" align="center" prop="imidaclothiz" />
      <el-table-column label="呋虫胺" align="center" prop="flonicamid" />
      <el-table-column label="虱螨脲" align="center" prop="lufenuron" />
      <el-table-column label="倍硫磷" align="center" prop="phosalone" />
      <el-table-column label="抑霉唑" align="center" prop="difenoconazole" />
...
<script>
import { listOutTeaNoBanPesDetRecords, getOutTeaNoBanPesDetRecords, delOutTeaNoBanPesDetRecords, addOutTeaNoBanPesDetRecords, updateOutTeaNoBanPesDetRecords } from "@/api/out/outTeaNoBanPesDetRecords";

export default {
  name: "OutTeaNoBanPesDetRecords",
  data() {
    return {
      // 遮罩层
      loading: true,
      ...
      // 总条数
      total: 0,
      // 茶叶上非禁止使用农药检出及超标情况表格数据
      outTeaNoBanPesDetRecordsList: [],
      // 弹出层标题
      title: "",
      // 是否显示弹出层
      open: false,
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listOutTeaNoBanPesDetRecords(this.queryParams).then(response => {
        this.outTeaNoBanPesDetRecordsList = response.rows;
    ...
    handleSelectionChange(selection) {
      this.ids = selection.map(item => item.teaNoBanPesDetRecordsId)
    ...
    handleAdd() {
      this.reset();
      this.open = true;
      this.title = "添加茶叶上非禁止使用农药检出及超标情况";
    },
    /** 修改按钮操作 */
    handleUpdate(row) {
      this.reset();
      const teaNoBanPesDetRecordsId = row.teaNoBanPesDetRecordsId || this.ids
      getOutTeaNoBanPesDetRecords(teaNoBanPesDetRecordsId).then(response => {
        this.form = response.data;
        this.open = true;
        this.title = "修改茶叶上非禁止使用农药检出及超标情况";
      });
    },
    /** 提交按钮 */
    submitForm() {
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.teaNoBanPesDetRecordsId != null) {
            updateOutTeaNoBanPesDetRecords(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addOutTeaNoBanPesDetRecords(this.form).then(response => {
    ...
    /** 删除按钮操作 */
    handleDelete(row) {
      const teaNoBanPesDetRecordsIds = row.teaNoBanPesDetRecordsId || this.ids;
      this.$modal.confirm('是否确认删除茶叶上非禁止使用农药检出及超标情况编号为"' + teaNoBanPesDetRecordsIds + '"的数据项？').then(function() {
        return delOutTeaNoBanPesDetRecords(teaNoBanPesDetRecordsIds);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('out/outTeaNoBanPesDetRecords/export', {
        ...this.queryParams
      }, `outTeaNoBanPesDetRecords_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
