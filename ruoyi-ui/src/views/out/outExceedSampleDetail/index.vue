...
<!--        />-->
<!--      </el-form-item>-->
<!--      <el-form-item label="查处情况" prop="actionTaken">-->
<!--        <el-input-->
<!--          v-model="queryParams.actionTaken"-->
<!--          placeholder="请输入查处情况"-->
<!--          clearable-->
<!--          @keyup.enter.native="handleQuery"-->
<!--        />-->
<!--      </el-form-item>-->
<!--      <el-form-item label="查处人" prop="investigator">-->
<!--        <el-input-->
...
<!--          size="mini"-->
<!--          @click="handleAdd"-->
<!--          v-hasPermi="['out:outExceedSampleDetail:add']"-->
<!--        >新增</el-button>-->
<!--      </el-col>-->
<!--      <el-col :span="1.5">-->
<!--        <el-button-->
...
<!--          :disabled="single"-->
<!--          @click="handleUpdate"-->
<!--          v-hasPermi="['out:outExceedSampleDetail:edit']"-->
<!--        >修改</el-button>-->
<!--      </el-col>-->
<!--      <el-col :span="1.5">-->
<!--        <el-button-->
...
<!--          :disabled="multiple"-->
<!--          @click="handleDelete"-->
<!--          v-hasPermi="['out:outExceedSampleDetail:remove']"-->
<!--        >删除</el-button>-->
<!--      </el-col>-->
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['out:outExceedSampleDetail:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="outExceedSampleDetailList" @selection-change="handleSelectionChange">
      <el-table-column type="selection" width="55" align="center" />
<!--      <el-table-column label="唯一标识符，自增" align="center" prop="exceedSampleDetailId" />-->
...
<!--      <el-table-column label="查处情况" align="center" prop="actionTaken" />-->
<!--      <el-table-column label="查处人" align="center" prop="investigator" />-->
...
<!--      </el-table-column>-->
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="查处情况" prop="actionTaken">
          <el-input v-model="form.actionTaken" placeholder="请输入查处情况" />
        </el-form-item>
        <el-form-item label="查处人" prop="investigator">
          <el-input v-model="form.investigator" placeholder="请输入查处人" />
        </el-form-item>
...
<script>
import { listOutExceedSampleDetail, getOutExceedSampleDetail, delOutExceedSampleDetail,
  addOutExceedSampleDetail, updateOutExceedSampleDetail,
  samplingAddressProvince,
  findBySamplingProvinceCode,
  findBySamplingCityCode, } from "@/api/out/outExceedSampleDetail";
import {listOutDlDetectRecords} from "@/api/out/outDlDetectRecords";

export default {
  name: "OutExceedSampleDetail",
  dicts: ['veg_fru_type'],
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listOutExceedSampleDetail(this.addDateRange(this.queryParams, this.dateRange)).then(response => {
            this.outExceedSampleDetailList = response.rows;
    ...
    handleSelectionChange(selection) {
      this.ids = selection.map(item => item.exceedSampleDetailId)
    ...
    handleAdd() {
      this.reset();
      this.open = true;
      this.title = "添加超标蔬菜水果样品明细";
    },
    /** 修改按钮操作 */
    handleUpdate(row) {
      this.reset();
      const exceedSampleDetailId = row.exceedSampleDetailId || this.ids
      getOutExceedSampleDetail(exceedSampleDetailId).then(response => {
        this.form = response.data;
        this.open = true;
        this.title = "修改超标蔬菜水果样品明细";
      });
    },
    /** 提交按钮 */
    submitForm() {
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.exceedSampleDetailId != null) {
            updateOutExceedSampleDetail(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addOutExceedSampleDetail(this.form).then(response => {
    ...
    /** 删除按钮操作 */
    handleDelete(row) {
      const exceedSampleDetailIds = row.exceedSampleDetailId || this.ids;
      this.$modal.confirm('是否确认删除超标蔬菜水果样品明细编号为"' + exceedSampleDetailIds + '"的数据项？').then(function() {
        return delOutExceedSampleDetail(exceedSampleDetailIds);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('out/outExceedSampleDetail/export', {
        ...this.queryParams
      }, `outExceedSampleDetail_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
